...
void AppConfig::Init(char **argv) {
  ...
  path_binary_ = App::GetBinaryPath(argv_);
  std::string path_config_file = path_binary_ + AppConfig::kFilename;

  if (HelperFile::FileExists(path_config_file)) {
    // Load existing config
    config_file_content_ = HelperFile::GetFileContents(path_config_file);
  } else {
...
std::string AppConfig::GetDefaultConfig() {
  ...
  content << ";TimesheetPlus Configuration"
          ...
          << "\n;report_path=" << App::GetBinaryPath(argv_)
          << "\n"
          ...
          << "\n;  Example 1: tsp u          -> opens url.default in web browser"
          << "\n;  Example 2: tsp u 123      -> opens url.default, with \"#TASK#\" replaced by \"123\" in web browser"
          << "\n;  Example 3: tsp u d        -> opens url.default, for all tasks in current day"
          << "\n;  Example 4: tsp u d -1     -> opens url.default, for all tasks in previous day"
          << "\n;  Example 5: tsp u 123 edit -> opens url.edit, with \"#TASK#\" replaced by \"123\" in web browser"
          << "\n;  Example 6: tsp u d edit   -> opens url.edit, with \"#TASK#\" replaced by \"123\" in web browser, etc."
...
void AppConfig::InitConfigMap() {
  ...
  std::vector<std::string> lines = HelperString::Explode(config_file_content_, '\n');
  for (std::vector<int>::size_type i = 0; i != lines.size(); i++) {
    std::string line = lines[i];
    if (!HelperString::StartsWith(line.c_str(), ";")) {
...
std::string AppConfig::GetDefaultLanguageKey() {
  std::string languageKey = HelperSystem::GetLanguageKey();
...
std::string AppConfig::GetConfigValueDefault(const std::string &key) {
  switch (ResolveOption(key)) {
    case Option_Cli_Theme:return HelperNumeric::ToString(GetDefaultThemeIdByOs());
...
int AppConfig::GetDefaultThemeIdByOs() {
  return HelperSystem::kOsName == "macOs"
