...
bool ReportRecalculator::Recalculate() {
  ...
  int minutes_per_day_should = HelperDateTime::GetSumMinutesFromTime(AppConfig::GetConfigValueStatic("debit_per_day"));
  int balance = 0;
  ...
  for (int index_row = 0; index_row <= amount_rows; index_row++) {
    ...
    if (is_new_day) {
      ...
      balance += debit_day;
      balance_formatted = HelperDateTime::GetHoursFormattedFromMinutes(balance);
      ReportHtmlParser::UpdateColumn(html_, index_row - 1, Report::ColumnIndexes::Index_Balance, balance_formatted);

      sum_day_formatted = HelperDateTime::GetHoursFormattedFromMinutes(sum_minutes_day);
    ...
    if (!current_duration.empty()) {
      int duration_minutes = HelperDateTime::GetSumMinutesFromTime(current_duration);
...
void ReportRecalculator::UpdateTaskSumsFromMaps(std::string &html) {
  for (auto const &task_row_index : task_in_day_last_index_) {
    ...
    std::string sum_formatted = HelperDateTime::GetHoursFormattedFromMinutes(sum_minutes);
