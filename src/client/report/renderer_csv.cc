...
std::string ReportRendererCsv::GetFilename(std::string html) {
  ...
  date_first = HelperDateTime::ReformatDateAsYyyyMmDd(date_first, report_date_format);
  ...
  date_last = HelperDateTime::ReformatDateAsYyyyMmDd(date_last, report_date_format);
...
bool ReportRendererCsv::RenderToFile(std::string path, RenderScopes scope) {
  ...
  if (!csv.empty()) {
    ...
    if (HelperFile::FileExists(filename)) std::remove(ReportFile::kFilenameReport.c_str());

    if (HelperFile::WriteToNewFile(filename, csv)) {
...
std::string ReportRendererCsv::RenderCsv(RenderScopes scope) {
  ...
  for (int index_column = 1; index_column < amount_columns_; index_column++) {
    csv_stream << "\"" << column_titles_[index_column] << "\""
               << (index_column == last_column ? HelperSystem::kNewline : ",");
  }
  ...
  for (int index_row = 0; index_row < amount_rows_ && index_cell < amountCells; index_row++) {
    for (int index_column = 0; index_column < amount_columns_ && index_cell < amountCells; index_column++) {
      if (index_column > 0) {
        csv_stream << "\"" << HelperString::CsvEncode(cells_[index_cell]) << "\"";
        csv_stream << (index_column == last_column ? ("\"" + HelperSystem::kNewline) : ",");
      }
      index_cell++;
    }
  }
