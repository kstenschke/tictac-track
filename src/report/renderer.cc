...
bool ReportRenderer::ExtractPartsFromReport(int filter_offset) {
  ...
  std::string html = HelperFile::GetFileContents(file);
  if (html.empty()) return false;

  html = HelperString::ReplaceAll(html, "\n", "");
  std::string table = HelperString::GetSubStrBetween(html, "<table border=\"1\">", "</table>");
  ...
  for (int index_column = 0; index_column < amount_columns_; index_column++) {
    ...
    column_content_max_len_.push_back(HelperString::GetUtf8Size(HelperHtml::Decode(column_titles_[index_column])));
  }
  ...
  amount_rows_ = HelperString::GetSubStrCount(table.c_str(), "<tr");
...
#pragma ide diagnostic ignored "missing_default_case"
    switch (render_scope_) {
      case Scope_Day:
        if (!HelperString::Contains(row, rows_filter_)) {
...
#pragma clang diagnostic pop
    ...
    row = HelperString::ReplaceAll(row, "<td>", "");
    row = HelperString::ReplaceAll(row, "<td class=\"meta\">", "");
    row = HelperString::ReplaceAll(row, "</td>", "|");
    row = HelperString::ReplaceAll(row, "||", "| |");

    std::vector<std::string> columns = HelperString::Explode(row, '|');
    int amount_columns_current = columns.size();

    if (columns.size() >= HelperNumeric::ToUnsignedInt(amount_columns_)) {
      for (int indexColumn = 0; indexColumn < amount_columns_; indexColumn++) {
        ...
        contentLen = indexColumn == Report::ColumnIndexes::Index_Comment
                     ? HelperNumeric::ToUnsignedInt(HelperHtml::Decode(columns[indexColumn]).size())
                     : HelperNumeric::ToUnsignedInt(columns[indexColumn].size());
...
std::string ReportRenderer::ExtractTheadFromTable(std::string &table) {
  std::string tHead = HelperString::GetSubStrBetween(table, "<thead>", "</thead>");
  ...
  amount_columns_ = HelperString::GetSubStrCount(tHead.c_str(), "<th");
...
void ReportRenderer::SetColumnTitlesExtractedFromTHead(std::string t_head) {
  t_head = HelperString::ReplaceAll(t_head, "<th class=\"meta\">", "");
  t_head = HelperString::ReplaceAll(t_head, "<th>", "");
  t_head = HelperString::ReplaceAll(t_head, "</th>", "|");

  column_titles_ = HelperString::Explode(t_head, '|');
}

std::vector<std::string> ReportRenderer::ExtractRowsFromTable(std::string table) {
  table = HelperString::ReplaceAll(table, "<tr>", "");
  table = HelperString::ReplaceAll(table, "<tr class=\"new-day\">", "");
  table = HelperString::ReplaceAll(table, "</tr>", "|");
